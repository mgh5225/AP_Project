...
void Login::on_btn_login_click(){
	...
	if (answer["code"].toBool()) {
		app->root()->clear();
		app->removeStyleSheet("main.css");
		app->removeStyleSheet("index.css");
		app->useStyleSheet("userPanel.css");
		Json::Object person = answer["person"];
		if (answer["mode"] == "user") {
			UserPanel* userPanel = new UserPanel(person);
			app->root()->addWidget(unique_ptr<WContainerWidget>(userPanel));
		}
		else if (answer["mode"] == "manager") {
			ManagerPanel* managerPanel = new ManagerPanel(person);
...
UserPanel::UserPanel(Json::Object person) {
	...
	my_G_layout = new WGridLayout;
	my_G_layout->setHorizontalSpacing(0);
	my_G_layout->setVerticalSpacing(0);
	my_G_layout->setContentsMargins(0, 0, 0, 0);
...
void UserPanel::read(const Json::Object& json) {
	...
	for (int i = 0; i < my_G_layout->count(); i++) {
		WWidget* widget = my_G_layout->itemAt(i)->widget();
	...
	for (int i = 0; flag; i++) {
		for (int j = 0; j < 4; j++) {
			...
			my_G_layout->addWidget(unique_ptr<Building>(blds[k++]), i, j);
		}
	}
}
...
void UserPanel::on_btn_logout_click() {
	...
	if (answer["code"].toBool()) {
		app->root()->clear();
		app->removeStyleSheet("main.css");
		app->removeStyleSheet("userPanel.css");
		app->useStyleSheet("index.css");
		firstPage* f = new firstPage;
...
ManagerPanel::ManagerPanel(Json::Object person) {
	...
	my_H_layout->addWidget(unique_ptr<WPushButton>(btn_showBalance), 1, AlignmentFlag::Left);
	my_H_layout->addWidget(unique_ptr<WGroupBox>(explore_group), 1, AlignmentFlag::Left);
...
Building::Building(Json::Object& json){
	...
	total_area = new WLabel(json["total_area"]);
	address = new WLabel(json["address"]);
	final_price = new WLabel(json["final_price"]);
...
AddNewFlat::AddNewFlat(Json::Object apartment){
