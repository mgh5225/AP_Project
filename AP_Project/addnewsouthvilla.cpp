...
{
    south_villa *n=new south_villa(lineEdit_BasePrice->text().toDouble(),lineEdit_FullArea->text().toLongLong(),lineEdit_Address->text(),fileName,lineEdit_NumberOfRooms->text().toInt(),lineEdit_BuildArea->text().toLongLong(),lineEdit_YardArea->text().toLongLong(),lineEdit_ParkingArea->text().toLongLong());
    svillas[n->get_id()]=*n;
    QJsonObject j;
    n->write(j);
